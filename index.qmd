...
Flight data include origin/destination aiport, aircraft type,
off-block and arrival times,...and estimated \acr{TOW}.
Thanks to the collaboration to \acr{OSN} we added the relevant trajectories 
at (max) 1 second granularity, accounting for around 158 GiB of parquet files.

The 105959 flights of the `submission_set_csv` will be used to rank the intermediate 
submissions before the deadline, and an additional 52190 that we will add for the 
final prize ranking.
...
::: {.callout-warning}
## Delay in handling Team creation requests

We are facing some minor issues for teams creation.  
Please bear with us and do not worry, we will process your request in few days.
:::
