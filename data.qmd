...
The `competition-data` is the S3 bucket as set up by \acr{OSN} for the Challenge.
...
(UTC) `<yyyy-mm-dd>` date, it then can happen that flight portions be present
in consecutive files, i.e. the same `flight_id` will occur in more than one 
daily file.

**NOTE**: trajectories are not necessarily complete with respect to what
reported in the flight list. This is due to the possibly limited/partial 
\acr{ADS-B} coverage in some parts (or some lower altitudes) of the world.
...
Please eventually manually **reload it**.
