...
The submission template file, `submission_set.csv`, contains the list of flight IDs
for which to estimate the TOW; it is .
...
Below you can find the details to access the data sets and submit your
results for ranking.


### Pre-requisites
The steps below have been executed on a MBR/macOS machine but it should easy
to apply it to other unix-like environments (we did similarly on MS Windows via
Git Bash.)
...
**NOTE**: no team can list the content of the `submissions/` bucket; and team `cucu` 
can only submit files in the format `cucu_submission_<posix timestamp>.csv`
