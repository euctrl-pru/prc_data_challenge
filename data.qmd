...
in 2022. Itprovides details such as (`column names` in parenthesis, [_units_]
...
### Trajectory
The trajectoris for the flight list for modeling and the ones for ranking are
...
the list of flight IDs for which to estimate the TOW; it looks something like:
...
all the flight IDs as present in the `competition-data/submission_set.csv`
...
use \acr{RMSE} to compare a submission for the 105959 flights in `submission_set.csv`
with the (hidden) the ground truth.
...
Teams (or individuals) must use their own original solutions.
...
filters to match the specific pecularities of the data, however, can be 
...
1. the open availabiliy of the code and the relevant documentation provided.

By participating in the Challenge, you agree to be bound by the final decision
of the Challenge selection committee. 
