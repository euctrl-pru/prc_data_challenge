...
We haven't filtered against OSN flight list but retrieved all
that is available for the relevant flights (and more to be frank).  
*Please consider re-downloading them*.

#### Updated trajectories on **24th Sep 2024**

**24th Sep 2024**: trajectory files have been reprocessed (weird constant altitudes
and midnight crossing issues).  
*Please consider re-downloading them*.

**16th Aug 2024**: `challenge_set.csv` now contains fixes for `tow` 
originally below minimum mass for the relevant aircraft types.  
...
1. the relevant trajectories for the above flights as recorded/processed by \acr{OSN} (**note**: few thousand flights have **no** trajectory available)
...
(**note**: few thousand flights have **no** trajectory available)
...
* 4D position: `longitude` [_DD_, decimal degrees in -180/180 range] and
  `latitude` [_DD_, decimal degrees in -90/90 range],
...
a flight list for which to estimate the \acr{TOW}; it has
two columns, namely `flight_id` and `tow` (but of course empty `tow`.)
  
The relevant trajectories associated to the `flight_id`'s are part of
the ones provided in the Trajectory dataset.
...
You can explore the trajectory data using the [`traffic`][traffic] library in
a Python notebook.
...
final portion at `2022-01-01T11:55:15Z` (and not that there is no very low altitude data from the takeoff).
...
**NOTE**: Missing `flight_id` rows will get a `tow` of 0 (zero),
as well as empty/null values for `tow`. 
...
submissions (`dc24` alias as explained above):
...
The [leaderboard][ranking] will display the best ranked submissions per team.   
Eventually manually **reload it**, please.

To query for all calculated RMSE's for all submissions, please use
